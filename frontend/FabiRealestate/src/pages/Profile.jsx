 ...
 /* useEffect(() => {
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const res = await fetch(
        `https://u09-fullstack-js-simretn.onrender.com/api/user/delete/${currentUser._id}`,
        { credentials: "include" },
        {
          method: "DELETE",
        }
  ...
  const handleSignOut = async () => {
    try {
      ...
      const res = await fetch(
        "https://u09-fullstack-js-simretn.onrender.com/api/auth/signout");
      const data = await res.json();
      if (data.success === false) {
        dispatch(deleteUserFailure(data.message));
        return;
  ...
  const handleListingDelete = async (listingId) => {
    try {
      const res = await fetch(
        `https://u09-fullstack-js-simretn.onrender.com/api/listing/delete/${listingId}`,
    ...
    <div className="p-4 max-w-lg mx-auto rounded-lg shadow-lg bg-white">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <img
          ...
          src={formData.avatar || currentUser.avatar}
          alt="Profile"
          className="rounded-full h-24 w-24 mx-auto cursor-pointer object-cover"
        />
        {fileUploadError && (
          <p className="text-red-500 text-center text-sm">Image upload error: File must be under 2 MB.</p>
        )}
        <input
          type="text"
          placeholder="Username"
        ...
        <button
          type="submit"
          className="w-full bg-blue-600 text-white p-2 rounded-md hover:bg-blue-700 focus:outline-none focus:bg-blue-700"
          disabled={loading}
        >
      ...
      <div className="mt-6 flex justify-between">
        <button onClick={handleDeleteUser} className="text-red-600">Delete Account</button>
